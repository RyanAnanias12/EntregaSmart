...
router.post('/checkout', auth, async (req, res) => {
  ...
  const priceId = plano === 'solo'
    ? (process.env.STRIPE_PRICE_SOLO || process.env.STRIPE_PRICE_PRO)
    : process.env.STRIPE_PRICE_PRO

  // inject plano into metadata below
  req._plano = plano
  req._priceId = priceId
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price: req._priceId || process.env.STRIPE_PRICE_PRO, quantity: 1 }],
      success_url: `${process.env.FRONTEND_URL}/dashboard?upgrade=ok`,
      cancel_url:  `${process.env.FRONTEND_URL}/precos`,
      customer_email: req.user.email,
      metadata: { tenant_id: String(req.user.tenant_id), plano: req._plano || 'pro' },
      locale: 'pt-BR',
    })
...
router.post('/webhook', async (req, res) => {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (tenant_id) {
        await pool.query(
          `UPDATE tenants SET plano=$1, stripe_customer=$2, stripe_sub=$3 WHERE id=$4`,
          [plano, session.customer, session.subscription, tenant_id]
        )
